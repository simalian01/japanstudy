...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startGame(level) {
        // 1. 读取并应用设置 (实时获取 Header 中的控件)
        const timerDurationSelect = document.getElementById('timer-duration');
        const nextModeRadios = document.querySelectorAll('input[name="next-mode"]');
        const delayCorrectSelect = document.getElementById('delay-correct');
        const delayIncorrectSelect = document.getElementById('delay-incorrect');

        // 健壮性检查：确认控件已找到
        if (!timerDurationSelect || !delayCorrectSelect || !delayIncorrectSelect) {
             console.error("错误：未能找到页眉中的设置控件（例如 #timer-duration）。页眉组件是否已加载？");
             // 在实际应用中，可能需要给用户更友好的提示或禁用开始按钮
             // alert("无法读取测验设置，请稍后重试或检查控制台。");
             // return; // 暂时不停止，允许使用默认值继续，但控制台会报错
        }

        // 尝试读取设置，如果控件不存在则使用默认值
        gameTimeLimit = timerDurationSelect ? (parseInt(timerDurationSelect.value) || 5) : 5;
        const nextModeSetting = document.querySelector('input[name="next-mode"]:checked'); // querySelector 本身是安全的
        autoNextMode = nextModeSetting ? nextModeSetting.value === 'auto' : true; // 默认为自动
        autoDelayCorrect = delayCorrectSelect ? ((parseFloat(delayCorrectSelect.value) || 2) * 1000) : 2000; // 默认 2 秒
        autoDelayIncorrect = delayIncorrectSelect ? ((parseFloat(delayIncorrectSelect.value) || 4) * 1000) : 4000; // 默认 4 秒
    ...
    function showLevelSelection() {
        // settingsArea.classList.remove('hidden');      // 显示设置区域 <-- 注释掉这行，因为 settingsArea 已移至 header
